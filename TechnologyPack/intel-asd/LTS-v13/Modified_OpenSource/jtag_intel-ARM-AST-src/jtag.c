...
void *jtag_priv_asd(struct jtag *jtag)
{
	return jtag->priv;
}
EXPORT_SYMBOL_GPL(jtag_priv_asd);
...
struct jtag *jtag_alloc_asd(struct device *host, size_t priv_size,
			const struct jtag_ops *ops)
{
	struct jtag *jtag;
...
EXPORT_SYMBOL_GPL(jtag_alloc_asd);

void jtag_free_asd(struct jtag *jtag)
{
	kfree(jtag);
}
EXPORT_SYMBOL_GPL(jtag_free_asd);
...
int devm_jtag_register_asd(struct device *dev, struct jtag *jtag)
{
	struct jtag **ptr;
	int ret;
...
EXPORT_SYMBOL_GPL(devm_jtag_register_asd);
