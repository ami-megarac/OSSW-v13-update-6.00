...
{
	struct aspeed_jtag *aspeed_jtag = jtag_priv_asd(jtag);
...
static int aspeed_jtag_probe(struct platform_device *pdev)
{
	...
	jtag = jtag_alloc_asd(&pdev->dev, sizeof(*aspeed_jtag),
			  jtag_functions->aspeed_jtag_ops);
	if (!jtag)
		return -ENOMEM;

	platform_set_drvdata(pdev, jtag);
	aspeed_jtag = jtag_priv_asd(jtag);
	...
	err = devm_jtag_register_asd(aspeed_jtag->dev, jtag);
	if (err)
...
static int aspeed_jtag_remove(struct platform_device *pdev)
{
	...
	aspeed_jtag_deinit(pdev, jtag_priv_asd(jtag));
	return 0;
}
